...
const narrowLayoutQuery = "(max-width: 640px)";
...
function PasswordView({ isDialogOpenRef, requirePasswordAuth }: PasswordViewProps) {
    ...
    return (
      <div className="pw-auth">
        <div className="pw-auth-card">
          <div className="pw-auth-icon">🔐</div>
          <div className="pw-auth-title">密码管理</div>
          <div className="pw-auth-desc">
            通过当前 Windows 用户密码验证后才能打开密码库。
          </div>
          {authError && <div className="pw-error">{authError}</div>}
          <button className="pw-primary-btn" onClick={unlockVault} disabled={isUnlocking}>
  ...
  const showDetail = isNarrowPasswordLayout && selectedDomain;
  const showListOnly = isNarrowPasswordLayout && !selectedDomain;

  return (
    <div
      className={`pw-app ${showDetail ? "pw-detail-only" : ""} ${
        showListOnly ? "pw-list-only" : ""
      }`}
    >
      <aside className="pw-rail">
        <div className="pw-rail-head">
          <div className="pw-rail-brand">
            <span className="pw-rail-mark">🔐</span>
            <h1 className="pw-rail-title">密码管理</h1>
          </div>
          <button className="pw-lock-btn" onClick={lockVault} title="锁定密码库">
            锁定
          </button>
        </div>

        <div className="pw-rail-search">
          <input
            className="pw-input"
            placeholder="搜索域名或账号..."
            value={domainSearch}
        ...
        <div className="pw-rail-add">
          ...
          />
          <button className="pw-add-btn" onClick={addDomain} title="添加域名">
            +
          </button>
        </div>
        ...
        <div className="pw-rail-list">
          {filteredDomains.length === 0 && (
            <div className="pw-empty pw-empty-sm">暂无域名</div>
          )}
          {filteredDomains.map((domain) => (
            <div
              key={domain.id}
              className={`pw-domain ${selectedDomainId === domain.id ? "active" : ""}`}
              onClick={() => openDomain(domain.id)}
            >
              <div className="pw-domain-main">
                <span className="pw-domain-name">{domain.domain}</span>
                <span className="pw-domain-count">{domain.accounts.length} 个账号</span>
              </div>
              <div className="pw-domain-actions">
                <button
                  className="pw-mini-btn"
                ...
                <button
                  className="pw-mini-btn pw-mini-danger"
      ...
      <main className="pw-content">
        <header className="pw-content-head">
          {showDetail && (
            <button className="pw-back-btn" onClick={showDomainList} title="返回域名列表">
              ← 域名列表
            </button>
          )}
          <div className="pw-content-title-wrap">
            <h2 className="pw-content-title">{selectedDomain?.domain ?? "选择一个域名"}</h2>
          ...
          {!showDetail && (
            <button className="pw-lock-btn pw-lock-btn-top" onClick={lockVault}>
              锁定
            </button>
          )}
        </header>

        <div className="pw-content-body">
          {saveError && <div className="pw-error">{saveError}</div>}

          {selectedDomain ? (
            <>
              <div className="pw-account-form">
                <input
                  className="pw-input"
                  placeholder="账号 / 用户名"
                  value={newUsername}
                ...
                />
                <input
                  className="pw-input pw-input-note"
                  placeholder="备注（可选）"
                  value={newNote}
                  onChange={(event) => setNewNote(event.target.value)}
                  onKeyDown={(event) => event.key === "Enter" && addAccount()}
                />
                <button className="pw-primary-btn" onClick={addAccount}>
                  添加账号
                </button>
              </div>

              <div className="pw-account-list">
                {selectedDomain.accounts.length === 0 && (
                  <div className="pw-empty">这个域名下还没有账号</div>
                )}
                {selectedDomain.accounts.map((account) => {
                  const isVisible = !!visiblePasswordIds[account.id];
                  return (
                    <div className="pw-account" key={account.id}>
                      <div className="pw-account-head">
                        <button
                          className="pw-account-user"
                          onClick={() => void copyAccount(account.username)}
                          title="复制账号"
                        >
                          {account.username}
                        </button>
                        <div className="pw-account-actions">
                          <button
                            className="pw-small-btn"
                            onClick={() => void copyAccount(account.username)}
                          >
                            复制账号
                          </button>
                          ...
                          <button
                            className="pw-small-btn pw-small-danger"
                      ...
                      <div className="pw-note">{account.note || "无备注"}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="pw-empty pw-empty-lg">左侧添加或选择一个域名</div>
          )}
        </div>
      </main>
    </div>
