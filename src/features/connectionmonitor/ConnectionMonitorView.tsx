...
type ConnectionMonitorViewProps = {
  onBack: () => void;
};
...
function buildConnectChartOption(samples: ConnectSample[]): EChartsOption {
  return {
...
function ConnectionMonitorView({ onBack }: ConnectionMonitorViewProps) {
  const [target, setTarget] = useState("127.0.0.1");
  const [port, setPort] = useState("80");
  const [frequency, setFrequency] = useState(2);
  const [samples, setSamples] = useState<ConnectSample[]>([]);
  ...
  useEffect(() => {
    portRef.current = portValid ? parsedPort : 0;
  }, [parsedPort, portValid]);
  ...
  useEffect(() => {
    ...
    chart.setOption(buildConnectChartOption(samples));
  ...
  const handleConnect = async () => {
    ...
    try {
      const result = await invoke<ConnectOnceResult>("connect_once", {
        request: {
  ...
  const canStart = Boolean(normalizedTarget) && portValid;

  return (
    <div className="sub-view">
      <SubViewHeader title="连接监控" onBack={onBack} />
      <div className="sub-view-content network-tool ping-monitor connection-monitor">
        <section className="network-target-panel">
          <label className="network-field">
            ...
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter" && !running && canStart) void startMonitoring();
              }}
          ...
          <div className="network-inline-fields">
            <label className="network-field">
              ...
              <input
                ...
                inputMode="numeric"
                onChange={(event) => {
                  setPort(sanitizePort(event.target.value));
                  if (running) setRunning(false);
                }}
                placeholder="1-65535"
              />
        ...
        <section className="network-card ping-chart-card">
          ...
          <div className="ping-chart-wrapper">
            ...
            {!samples.length && (
              <div className="ping-chart-empty">点击“开始监控”后开始记录连接延迟数据</div>
            )}
          </div>
        </section>
      </div>
...
export default ConnectionMonitorView;
